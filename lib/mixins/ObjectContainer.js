        ...
        return [
          h('v-stepper-content',
            ...
            [h('v-card', { props: { tile: true, flat: true } }, [
              ...
              isLast ? null : h('v-card-actions', { class: { 'px-0': true } }, [h('v-btn', {
                props: { color: 'primary' },
                on: {
                  click: () => {
              ...
              }, this.fullOptions.messages.continue)])
            ])]
          )
        ]
      } else {
    ...
    renderObjectContainer(h) {
      ...
      if (this.display === 'stepper' && sectionsChildren.length) {
        ...
        sectionsChildren = [h('v-stepper', { props, style: 'width: 100%;', on: { change: value => { this.currentStep = value } } }, [
          // Removing unwanted stepper-header
      ...
      return [h('v-row', { style: 'height: 100%', class: `ma-0 ${this.fullOptions.objectContainerClass}` }, [
